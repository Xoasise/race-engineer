...
# Note: message_content est un intent privilégié inutile ici car
# le bot n'utilise que des slash commands — on ne l'active pas.
...
# Guild ID optionnel : si défini, les commandes sont sync instantanément sur ce serveur.
# Laisser vide pour un sync global (peut prendre jusqu'à 1h chez Discord).
...
async def wrc_scan(interaction: discord.Interaction,
    ...
    # On convertit le slug Sportity (ex: "WRCSWE26") en clé courte (ex: "SWE")
    # pour que le polling automatique retrouve l'URL lors du rallye.
    ...
    if sportity_slug:
        ...
        config.save()
        log.info("Saved Sportity URL for rally key: %s (slug: %s)", rally_key, sportity_slug)
...
async def on_ready():
    ...
    try:
        if _TEST_GUILD:
            # Sync instantané sur le serveur ciblé (recommandé en prod/dev)
            ...
            # Sync global — peut prendre jusqu'à 1h pour propager chez Discord
            synced = await bot.tree.sync()
            log.info("Synced %d slash command(s) globally.", len(synced))
    except Exception as exc:
        log.error("Failed to sync commands: %s", exc)

    # Garde contre les doubles démarrages lors d'une reconnexion
    if not polling_task.is_running():
        polling_task.start()
        log.info("Polling task started.")
    else:
        log.info("Polling task already running — skipping start.")
