...
# Channels pour le post automatique des standings
...
async def post_standings_task():
    ...
    Si oui, publie automatiquement les classements pilotes et constructeurs.
    """
    ...
    for rally in calendar.all_rallies():
        ...
        if window_start <= now < window_end:
            log.info("Auto-posting standings after %s…", rally["name"])
...
async def on_ready():
    ...
    try:
        if _TEST_GUILD:
            # Vider les commandes globales résiduelles → supprime les doublons
            bot.tree.clear_commands(guild=None)
            await bot.tree.sync(guild=None)
            # Sync uniquement sur le guild cible — instantané
